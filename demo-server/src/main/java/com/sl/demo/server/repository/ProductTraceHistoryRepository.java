...
public interface ProductTraceHistoryRepository extends JpaRepository<ProductTraceHistory,Long>, JpaSpecificationExecutor<ProductTraceHistory> {
    ...
    @Query("select nt from ProductTraceHistory nt where nt.productCode = (?1) and nt.traceCode = (?2) order by nt.createDate desc ")
    ...
    @Query("select nt from ProductTraceHistory nt where nt.productCode = (?1) and nt.scanCode = (?2)")
    List<ProductTraceHistory> findHistoriesByScanCode(String productCode, String scanCode);

    @Query(value = "select nt.* from sl_product_trace_history nt where nt.product_code = (?1) and nt.scan_code = (?2) order by nt.create_date asc limit 1", nativeQuery=true)
